...
def find_website(cu_name):
    ...
    payload = {
        ...
        "messages": [
            ...
            {"role": "user", "content": f"Return this Australian credit union's website {cu_name}"},
        ],
        "max_tokens": 15,
        "temperature": 0.2,
        "top_p": 0.9,
...
def create_credit_union(name):
    desired_tag = name.lower().strip().replace(" ", "_") + "_australia"
...
australian_cus = []
with open(DIR + "australian_cus.txt", "r") as file:
    australian_cus = [line.strip() for line in file]

for name in australian_cus:
    try:
